...
export class CartComponent {
  ...
  form = this.fb.nonNullable.group({
    ...
    phone: [
      "",
      [Validators.required, Validators.pattern(/^[+]?[0-9][0-9\s-]{7,17}$/)],
    ],
    addressLine1: [
      "",
    ...
    this.orderService
      ...
      .subscribe({
        next: (res) => {
          ...
          const message =
            `*Brush Bloom Handmade* — New Order Request\n\n` +
            `Order ID: ${res.orderCode}\n` +
            `Name: ${customerName}\n` +
            `Address: ${fullAddress}\n\n` +
            `Items:\n${lines}\n\n` +
          ...
          this.toast.success("Order request submitted!");
        },
        error: (err) => {
          ...
          this.submitting.set(false);
          this.toast.error("Something went wrong submitting your order.");
        },
      });
  }
